      ...
      {assets.map((asset) => {
        ...
        return (
          <div
            ...
            className="bg-white dark:bg-[#111722] border border-[#E2E6EC] dark:border-[#283243] rounded-xl overflow-hidden shadow-2xs hover:shadow-sm hover:border-[#2859D9]/20 dark:hover:border-[#6F96FF]/20 cursor-pointer group flex flex-col justify-between transition-all duration-200"
          >
            ...
            <div className="relative aspect-video bg-[#F6F7F9] dark:bg-[#090D14] flex items-center justify-center border-b border-[#E2E6EC] dark:border-[#283243] overflow-hidden">
              ...
              {asset.duration && (
                <span className="absolute bottom-2 right-2 text-[8px] bg-zinc-950 text-white font-bold px-1 py-0.2 rounded-xs">
                  {asset.duration}
                </span>
              )}

              {isPortrait && (
                <span className="absolute top-2 right-2 text-[8px] border border-[#2859D9]/30 text-[#2859D9] bg-[#2859D9]/10 px-1.5 py-0.2 rounded-xs font-bold uppercase">
                  9:16 Port
                </span>
              )}
              ...
              {/* Processing Overlay */}
              {isProcessing && (
                <div className="absolute inset-0 bg-black/60 flex flex-col items-center justify-center p-3 text-center space-y-1.5 animate-fadeIn">
                  <RefreshCw className="w-5 h-5 text-white animate-spin" />
                  <span className="text-[9px] font-bold text-white uppercase tracking-wider">
                    {asset.status === "Transcoding" ? `Transcoding ${asset.progress}%` : "Uploading..."}
                  </span>
              ...
              {/* Failed Overlay */}
              {isFailed && (
                <div className="absolute inset-0 bg-red-950/60 dark:bg-red-950/80 flex flex-col items-center justify-center p-3 text-center space-y-1">
                  <AlertCircle className="w-5 h-5 text-red-500" />
                  <span className="text-[8px] font-bold text-red-400 uppercase tracking-wider leading-none">
                    Upload Failed
                  </span>
                  <span className="text-[7px] text-red-300 leading-snug line-clamp-2">
                    {asset.reason}
                  </span>
                </div>
              )}
            ...
            <div className="p-3 space-y-1 min-w-0">
              <span className="block text-[11px] font-bold text-zinc-900 dark:text-zinc-50 truncate leading-tight group-hover:text-[#2859D9] dark:group-hover:text-[#6F96FF]">
                {asset.name}
              </span>
              <div className="flex justify-between items-center text-[9px] text-zinc-400 dark:text-zinc-550 font-semibold">
